...
#+TITLE: Adam Taylor's  Emacs Initialization
   ...
   #+BEGIN_SRC emacs-lisp :results silent :exports results :var custom-vars=() :tangle no
     (cons '(Symbol\ Name Current\ Value Documentation)
           (cons 'hline
                 (cl-loop for cv in custom-vars collect `(,cv
                                                          ,(symbol-value cv)
                                                          ,(if (stringp (get cv 'variable-documentation))
                                                               (car (split-string (get cv 'variable-documentation) "\n"))
                                                             "")))))
   #+END_SRC
...
** ~init.el~ explained
   ...
   (if (file-newer-than-file-p "~/.emacs.d/README.org" "~/.emacs.d/README.el") ;; 7
