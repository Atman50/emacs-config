...
#+TITLE: Adam Taylor's  Emacs Initialization
   ...
   #+BEGIN_SRC emacs-lisp :results silent :exports results :var custom-vars=() :tangle no
     (cons '(Configured\ Variable\ Name Current\ Variable\ Value)
           (cons 'hline
                 (cl-loop for cv in custom-vars collect `(,cv ,(symbol-value cv)))))
   #+END_SRC
...
** ~init.el~ explained
   ...
   This code depends upon the following custom variable(s):
